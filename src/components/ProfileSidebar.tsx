...
export const ProfileSidebar = ({ isOpen, onClose, profileData }: ProfileSidebarProps) => {
  const [notes, setNotes] = useState("testing");

  if (!profileData) return null;

  return (
    <div className={`fixed top-0 right-0 h-full bg-white border-l border-gray-200 shadow-2xl transition-transform duration-300 ease-in-out z-50 ${
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              onClick={onClose}
              className="rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 p-1 hover:bg-gray-100"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          {/* Add Label Button */}
          <Button variant="outline" size="sm" className="w-full mb-4 text-gray-600 border-gray-300">
